...
class CustomDataset(Dataset):
    def __init__(self, questions, image_folder, tokenizer, image_processor, model_config, processor, scienceqa=False, single_pred_prompt=False, no_vision_input=False):
    ...
    def __getitem__(self, index):
        ...
        if self.scienceqa:
            ...
            qs = line["text"]
        if 'image' in line and self.no_vision_input:
            del line["image"]
        if 'image' in line:
...
def create_data_loader(model_base, questions, image_folder, tokenizer, image_processor, model_config, processor, scienceqa=False, single_pred_prompt=False, no_vision_input=False, batch_size=1, num_workers=16):
    assert batch_size == 1, "batch_size must be 1"
    dataset = CustomDataset(questions, image_folder, tokenizer, image_processor, model_config, processor, scienceqa, single_pred_prompt, no_vision_input)
...
def eval_model(args):
    ...
    with open(answers_file, "w") as ans_file:
        data_loader = create_data_loader(args.model_base, questions, args.image_folder, tokenizer, image_processor, model.config, processor, args.scienceqa, args.single_pred_prompt, args.no_vision_input)
        if "llava-1.5" in args.model_base or "llava-interleave" in args.model_base:
            for (input_ids, pixel_values, attention_mask), line in tqdm(zip(data_loader, questions), total=len(questions)):
                ...
                if args.scienceqa:
                    ...
                    if 'image' in line:
                        cur_prompt = '<image>' + '\n' + cur_prompt
                    if args.single_pred_prompt:  # MCQA format
...
if __name__ == "__main__":
    ...
    parser.add_argument("--scienceqa", type=bool, default=False)
