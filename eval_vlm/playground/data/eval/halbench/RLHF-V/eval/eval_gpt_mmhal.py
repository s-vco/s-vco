...
if __name__ == '__main__':
    ...
    parser.add_argument('--gpt-model', type=str, default='gpt-4-turbo')
    ...
    if args.evaluation is not None and os.path.exists(args.evaluation):
        with open(args.evaluation, 'r') as f_in:
            existing_responses = json.load(f_in)
            print(f"Skipping {len(existing_responses)}...", flush=True)
            records = records[len(existing_responses):]
            print(f"Left {len(records)} to run eval...", flush=True)
    ...
    for i, record in tqdm(enumerate(records), total=len(records)):
        ...
        # save responses after each output
    ...
    # analyze responses
    scores = []
    responses_tosave = copy.deepcopy(responses)
    for i, response in enumerate(responses):
        ...
        if len(scores_found) == 1:
            ...
            while len(scores_found) < 1:
                print(f"'none scores found - so: try-time: {retry_times} - rerunning {i}...", flush=True)
                print(i, response)
                ...
                while response_rerun is None:
                    try:
                        response_rerun = chat.chat_completion(
                            messages=[
                                {"role": "user", "content": input_text}
                            ],
                        ...
                        time.sleep(5)
                        continue
                responses_tosave[i] = response_rerun
                response = response_rerun['choices'][0]['message']['content']
                scores_found = []
                for s in range(7):
                    # if f'rating: {s}' in response.lower():
                    pattern = rf"(?:\*\*)?rating(?:\*\*)?\s*:?\s*{re.escape(str(s))}"
                ...
                if len(scores_found) == 1:
                    ...
                    print(f"'none scores found - reruned - still none - rerun again {i}...", flush=True)
                    print(i, response)
                    temperature += 0.2
                    retry_times += 1
                    
                    if retry_times >= 3:
                        print(f"'none scores found - reruned - retried 3 times - still none - set as 0 {i}...", flush=True)
    ...
    if args.evaluation is not None:
        with open(args.evaluation, 'w') as f:
            json.dump(responses_tosave, f, indent=2)

    if not len(scores) == 96:
        print(f"SCORES WRONG LENGTH from file: {args.response}", flush=True)
        scores = [0] * 96
