...
def get_file_names(args, model, sub_set_name):
    ...
    grade_file = f"{args.gpt_model}-grade-{args.num_run}runs.json"
    grade_file = os.path.join(args.result_path, grade_file)

    # score results regarding capabilities/capability integration to save
    cap_score_file = (
        f"{sub_set_name}{args.gpt_model}-cap-score-{args.num_run}runs.csv"
    )
    cap_score_file = os.path.join(args.result_path, cap_score_file)
    cap_int_score_file = f"{sub_set_name}{args.gpt_model}-cap-int-score-{args.num_run}runs.csv"
        ...
        for j in range(args.num_run):
            ...
            for id, line in tqdm(data.items()):
                ...
                while not grade_sample_run_complete:
                    try:
                        # CHANGED: replaced client.chat.completions.create with openai.ChatCompletion.create
                        ...
                        while flag:
                            try:
                                ...
                                try_time += 1
                                temperature += 0.5
                                print(f"{id} try {try_time} times -- content: {content}", flush=True)
                                print("sleep 5s", flush=True)
                                time.sleep(5)
                                # if try_time > 5:
                                #     score = 0.0
                                #     flag = False
                ...
                # if you prefer response["model"], you can do that, but response.model works in 0.28.x
...
if __name__ == "__main__":
    args = arg_parser()
    # CHANGED: remove creation of client, just set openai.api_key
